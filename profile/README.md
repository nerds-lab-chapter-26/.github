<div align="center">

# 👾 Nerds Lab

### *Two nerds. One GitHub org. Zero adult supervision.*

[![Nerd Level](https://img.shields.io/badge/nerd_level-over_9000-blueviolet)](#)
[![Coffee Powered](https://img.shields.io/badge/runs_on-coffee%20%26%20stackoverflow-yellow)](#)
[![Bugs Fixed](https://img.shields.io/badge/bugs-mostly%20fixed-success)](#)

</div>

---

## 🤓 Who are we?

We're **2 Nerds** who got tired of doing things the boring, manual way — so we automate it, script it, and ship it instead. No fancy pitch decks, no "synergy," just two people who like solving small annoying problems really, really well.

If it can be turned into a script, a CLI, or an unnecessarily over-engineered tool — we've probably already opened a terminal for it.

## 🛠️ Our tech diet

```
Frontend:  React, Next.js, whatever renders fastest
Backend:   Node.js, small scripts that quietly run the world
Debugging: console.log() and blind faith
Snacks:    mandatory
```

## ⭐ Why star our stuff?

Every ⭐ makes a nerd's day noticeably better. It costs you nothing and it's basically a virtual high-five. We'll take it.

## 📬 Say hi

Found a bug in one of our repos? Have an idea? Just want to talk about how much better life is with automation? Open an issue — we actually read them.

<div align="center">

*Made by 2 nerds, for anyone who hates doing repetitive stuff by hand.*

</div>
